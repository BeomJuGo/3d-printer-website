const STEPS = [
  { step: "01", title: "요구사항 상담", description: "카카오톡으로 원하시는 제품의 사진, 도면, 또는 간단한 설명을 보내주세요. 크기, 재질, 색상 등 세부 사항을 함께 논의합니다.", icon: "💬", color: "from-orange-500 to-amber-500" },
  { step: "02", title: "시제품 확인 & 수정", description: "요구사항을 바탕으로 3D 모델링 후 시제품을 제작합니다. 고객님의 피드백을 반영해 완성도를 높입니다.", icon: "🔧", color: "from-amber-500 to-yellow-500" },
  { step: "03", title: "최종 제작 & 배송", description: "최종 승인 후 제품을 정밀 제작하여 안전하게 포장 후 배송해 드립니다. 만족하실 때까지 책임집니다.", icon: "📦", color: "from-yellow-500 to-orange-400" },
];

export default function HowItWorks() {
  return (
    <section id="how-it-works" className="py-24 bg-[#0f0f0f]/92">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {STEPS.map((step, index) => (
              <div key={step.step} className="relative text-center group">
                <div className={`w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br ${step.color} flex items-center justify-center text-3xl shadow-lg group-hover:scale-110 transition-transform`}>{step.icon}</div>
                <div className="absolute top-0 right-1/2 translate-x-1/2 -translate-y-2">
                  <span className="text-xs font-bold text-orange-500/50 bg-orange-500/10 px-2 py-0.5 rounded-full">STEP {step.step}</span>
                </div>
                <h3 className="text-xl font-black text-white mb-3">{step.title}</h3>
                <p className="text-gray-400 leading-relaxed text-sm">{step.description}</p>
                {index < STEPS.length - 1 && (<div className="md:hidden w-0.5 h-8 bg-gradient-to-b from-orange-500 to-transparent mx-auto mt-6" />)}
              </div>
            ))}
          </div>
        </div>
